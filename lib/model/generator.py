...
class Generator2D(nn.Module):
    def __init__(self, z_dim, n_layers, init_res):
        super().__init__()
        
        self.n_layers = n_layers
        h_dim = 256
        # self.shape_code = nn.Parameter(0.002*torch.randn(1, h_dim, 2, 8,8),requires_grad=True)
        self.shape_code = nn.Parameter(0.002*torch.randn(1, h_dim, init_res, init_res),requires_grad=True)
    ...
    def forward(self, z):
        ...
        h = h.reshape(b, 2, -1, h.size(-2), h.size(-1))
        xy = h[:, 0, ...]
        yz = h[:, 1, ...][..., :h.size(-1)//2]
        xz = h[:, 1, ...][..., h.size(-1)//2:]
