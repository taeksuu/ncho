...
class ImplicitNetwork(nn.Module):
    def __init__(
            ...
            latent_cond_dim=-1,
            latent_embed_dim=-1,
            latent_obj_cond_layer=[],
            latent_obj_cond_dim=-1,
            latent_obj_embed_dim=-1,
            feat_cond_dim=0,
            feat_cond_layer=[],
            feat_cond_dim_2=0,
            feat_cond_layer_2=[],
        ...
        self.latent_obj_cond_layer = latent_obj_cond_layer
        self.latent_obj_cond_dim = latent_obj_cond_dim
        self.latent_obj_embed_dim = latent_obj_embed_dim
        if len(latent_obj_cond_layer)>0:
            self.cond_names.append('latent_obj')
        if latent_obj_embed_dim > 0:
            self.lin_p3 = nn.Linear(latent_obj_cond_dim, latent_obj_embed_dim)
            self.latent_obj_cond_dim = latent_obj_embed_dim


        self.feat_cond_layer = feat_cond_layer
        self.feat_cond_dim = feat_cond_dim
        self.feat_cond_layer_2 = feat_cond_layer_2
        self.feat_cond_dim_2 = feat_cond_dim_2
        ...
        for l in range(0, self.num_layers - 1):
            ...
            if l in self.latent_obj_cond_layer:
                input_dim += self.latent_obj_cond_dim
            if l in self.feat_cond_layer:
                input_dim += self.feat_cond_dim
            if l in self.feat_cond_layer_2:
                input_dim += self.feat_cond_dim_2
        ...
        self.istanh = istanh
        if self.istanh: self.tanh = nn.Tanh()

    def forward(self, input, cond, input_feat=None, input_feat_2=None, mask=None, return_feat=False, spatial_feat=False, val_pad=0, normalize=False, custom_3d=None):
        ...
        if input_dim == 3:
            ...
            if len(self.feat_cond_layer_2) > 0:
                input_feat_2 = input_feat_2[mask]
        ...
        if len(self.latent_obj_cond_layer) > 0:
            input_latent_obj_cond = cond['latent_obj']
            if self.latent_obj_embed_dim>0:
                input_latent_obj_cond = self.lin_p3(input_latent_obj_cond)     
        ...
        for l in range(0, self.num_layers - 1):
            ...
            if l in self.latent_obj_cond_layer:
                x = torch.cat([x, input_latent_obj_cond], dim=-1)

            if l in self.feat_cond_layer:
                x = torch.cat([x, input_feat], dim=-1)

            if l in self.feat_cond_layer_2:
                x = torch.cat([x, input_feat_2], dim=-1)
            ...
            if l < self.num_layers - 2:
                if self.istanh and l == self.num_layers - 3:
                    x = self.tanh(x)
                else:
                    x = self.softplus(x)
                # x = self.softplus(x)
