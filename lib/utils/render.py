...
class Renderer():
    def __init__(self, image_size=256,anti_alias=False,real_cam=False):
        ...
        t = torch.from_numpy(np.array([[0., 0.3, 2.0]])).float().to(self.device)
    ...
    def render_mesh_dict(self, mesh_dict, mode='npat'):
        ...
        with torch.no_grad():
            ...
            if 'a' in mode: 
                ...
                mesh_albido = Meshes(mesh_dict['verts'][None], mesh_dict['faces'][None], textures=Textures(verts_rgb=mesh_dict['verts_color'][None]))
